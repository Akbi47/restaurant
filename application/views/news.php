				...
				<div class="col-sm-9">
					...
					<?php foreach ($dulieutin as $motdl) : ?>
						...
						<div class="mottinchuan mb-3  wow  fadeInUp">
							...
							<a href="<?= base_url(); ?>tintuc/chiTietTin/<?= $motdl['id'] ?>"><img src="<?= $motdl['anhtin'] ?>" alt=""></a>
							<a href="<?= base_url(); ?>tintuc/chiTietTin/<?= $motdl['id'] ?>" class="tieudetin1 fontoswarld"><?= $motdl['tieude'] ?></a>
							<div class="ngaythang1"><?= date('d/m/Y - G:i - A', $motdl['ngaytao']) ?> In <span class="vang"> <?= $motdl['name'] ?></span></div>

							<p class="fontroboto"><?= $motdl['trichdan'] ?> </p>

							<div class="docthem mb-2">
								<div class="like float-xs-right fontroboto">10 like</div>
								<a href="<?= base_url(); ?>tintuc/chiTietTin/<?= $motdl['id'] ?>" class="rm fontroboto">Xem thêm</a>
							</div>
						</div>
					...
					<nav class="phantrang mb-3  wow  fadeInUp">
						<ul class="pagination">
							...
							?>
								<li class="pre">
									<a href="<?php echo base_url() ?>tintuc/page/<?php echo $tranghientai - 1 ?>" >
							...
							?>
								<li class="next">
									<a href="<?php echo base_url() ?>tintuc/page/<?= $tranghientai + 1 ?>" >
					...
					<div class="khoilistlink my-2  wow  fadeInUp">
						...
						<ul class="fontroboto">
							<?php foreach ($cacdanhmuc as $value) : ?>
								...
								<li><a href="<?= base_url(); ?>/tintuc/danhmuc/<?= $value['id']; ?>"> <?= $value['name']; ?></a></li>
							<?php endforeach ?>
